...
| `include_opencode_learning_inbox` | bool | `false` | **conditional** (`opencode` selected). Enable the OpenCode Learning-Inbox — a git-tracked staging layer that replaces the private-tier role OpenCode lacks natively. A SessionEnd plugin (`capture-learnings.ts`) auto-extracts learning proposals from the transcript via the configured `small_model` (Scribe); `/capture review` promotes kept proposals manually. Opt-in per Pareto/Security-by-default — the Scribe call sends a transcript excerpt to your `small_model` (off-host with Ollama Cloud). Disable at runtime via `OPENCODE_LEARNINGS_MIN_BYTES=0`. See [Learning-Inbox](explanation/learning-loop.md). |
