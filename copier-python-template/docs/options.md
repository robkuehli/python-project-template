...
| `python_version` | choice | `3.13` | `3.12` / `3.13` only. Newer releases are held back because Ruff, mypy, and pylint trail upstream Python by a few months on `target-version`/`py-version` flags — bump the template once the linter stack catches up. |
...
| `aider_model` | str | `claude-sonnet-4-6`, or `ollama/qwen3-coder:30b` on Ollama | **conditional** (`aider` selected). Default model alias. |
...
`opencode.json`) stays in lockstep. Edit them in `copier.yml`; do not duplicate
them per tool.
